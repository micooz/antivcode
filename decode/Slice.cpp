...
Slice::Slice(IImage* base, uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2)//,bool copied)
...
char
Slice::getSymbol()const {
    return _symbol;
}

uint32_t
Slice::getWidth()const {
    return _x2 - _x1;
}

uint32_t
Slice::getHeight()const {
    return _y2 - _y1;
}
...
void
Slice::setCode(const AttributeCode &code) {
    _code = code;
}

void
Slice::setSymbol(char symbol) {
    _symbol = symbol;
}

float
Slice::similarTo(Slice &slice) {
...
void
Slice::save(const std::string& file) {
    if (_data) {
        Bitmap bmp;
